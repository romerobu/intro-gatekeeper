...
In this case, constraint is limitating the resources a Pod can request (memory and cpu) within "gatekeeper-project" namespace. As memory and cpu resources request can be measured in different units it would
...
In this case we are going to test how to limit constraints scope within the cluster and across namespaces.
First of all some namespaces have been excluded via Config resource for webhook actions. This means we can audit and sync resources in this namespace but webhook won't execute on it.
To test this use case we have create a constraint matching the whole cluster less the excluded namespace.
