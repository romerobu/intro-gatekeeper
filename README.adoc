...
Constraints define:
 ...
 - Excluded namespace: apply the constraint to a non listed namespace, this applies only for this constraint and not for the whole set of constraints.
...
In this case, constraint is limitating the resources a Pod can request (memory and cpu) within the whole cluster less excluded namespace "gatekeeper-project-excluded" namespace. As memory and cpu resources request can be measured in different units it would
...
=== Restrictions without auditing a specific resource
